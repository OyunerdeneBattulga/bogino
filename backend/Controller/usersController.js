...
exports.login = async (req, res) => {
    try {
        ...
        if (!existingUser) {
            return res.status(404).json({ message: 'email buruu baina' });
        }
        const matchPassword = await bcrypt.compare(password, existingUser.password);
        if (!matchPassword) {
            return res.status(404).json({ message: 'nuuts ug buruu baina' });
        }
        const token = jwt.sign({ email: existingUser.email, id: existingUser._id, message: 'nevterlee' }, SECRET_KEY);
        console.log(existingUser);
        if (existingUser.role.enum === 'admin') {
            const admin = jwt.verify(
                { email: existingUser.email, id: existingUser._id, message: 'admin nevterlee' },
                SECRET_KEY,
            );
            res.status(200).json({ user: admin, token });
        } else {
            const user = jwt.verify(
                { email: existingUser.email, id: existingUser._id, message: 'user nevterlee' },
                SECRET_KEY,
            );
